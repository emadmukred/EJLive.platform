...
namespace EJLive.Server.WinForms
{
    public partial class ATMDetailDrawerForm : Form
    {
        ...
        private TabPage BuildOverviewTab()
        {
            var tab = new TabPage("📊  نظرة عامة") { BackColor = Color.FromArgb(25, 25, 30) };
            var panel = new TableLayoutPanel
            {
                Dock = DockStyle.Fill,
                ColumnCount = 2,
            ...
            void AddRow(string label, string value, Color? valueColor = null)
            {
                var lbl = new Label { Text = label, ForeColor = Color.FromArgb(117, 117, 117), Font = new Font("Segoe UI", 9.5F), Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleRight, Padding = new Padding(0, 0, 12, 0) };
                var val = new Label { Text = value, ForeColor = valueColor ?? Color.FromArgb(224, 224, 224), Font = new Font("Segoe UI", 9.5F, FontStyle.Bold), Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleLeft };
                panel.Controls.Add(lbl);
                panel.Controls.Add(val);
            }
            var status = _atm.ConnectionStatus == ConnectionStatus.Connected ? "متصل ✅" : "غير متصل ❌";
        ...
        private TabPage BuildEJLogTab()
        {
            var tab = new TabPage("📜  سجل EJ") { BackColor = Color.FromArgb(25, 25, 30) };
            var searchPanel = new Panel
            {
                Dock = DockStyle.Top,
                Height = 42,
            ...
            var txtSearch = new TextBox
            {
                PlaceholderText = "بحث في الجورنال...",
            ...
            var richLog = new RichTextBox
            {
                ...
                ForeColor = Color.FromArgb(180, 224, 180),
                Font = new Font("Courier New", 9F),
                ReadOnly = true,
                BorderStyle = BorderStyle.None,
                Text = $"// سجل EJ للصراف: {_atm.ATMId}\r\n// اختر معيار بحث أو اكتب نصًا للبحث\r\n"
        ...
        private TabPage BuildFaultsTab()
        {
            var tab = new TabPage("⚠  الأعطال") { BackColor = Color.FromArgb(25, 25, 30) };
            var lv = new ListView
            {
                Dock = DockStyle.Fill,
                View = View.Details,
