...
namespace EJLive.Server.WinForms
{
    public partial class JournalViewerForm : Form
    {
        ...
        {
            // ═══ شريط الأدوات ═══
            _toolPanel = new Panel { Height = 90, Dock = DockStyle.Top, BackColor = Color.FromArgb(26, 28, 30), Padding = new Padding(10, 8, 10, 8) };
            // أزرار البحث السريع
            var filterNames = new[]
            {
                "✅ Approved", "⚡ Power Up Reset", "🚨 Error E3", "💵 Cash Error",
            ...
            _filterBtns = new Button[filterNames.Length];
            var filterFlow = new FlowLayoutPanel { Height = 36, Dock = DockStyle.Top, FlowDirection = FlowDirection.LeftToRight, WrapContents = false };
            for (int i = 0; i < filterNames.Length; i++)
            {
                var idx = i;
                var btn = new Button
            ...
            // بحث نصي
            var searchRow = new FlowLayoutPanel { Height = 36, Dock = DockStyle.Bottom, FlowDirection = FlowDirection.LeftToRight };
            _txtSearch = new TextBox { Width = 300, BackColor = Color.FromArgb(40, 42, 45), ForeColor = Color.White, BorderStyle = BorderStyle.FixedSingle, Text = "بحث حر..." };
            _txtSearch.TextChanged += (s, e) => ApplyFreeSearch();
            var btnCopy = new Button { Text = "📋 نسخ", Width = 80, Height = 28, BackColor = Color.FromArgb(30, 50, 80), ForeColor = Color.White, FlatStyle = FlatStyle.Flat, Cursor = Cursors.Hand };
            btnCopy.Click += (s, e) => { if (!string.IsNullOrEmpty(_rtbJournal.SelectedText)) Clipboard.SetText(_rtbJournal.SelectedText); };
            var btnExport = new Button { Text = "📥 تصدير", Width = 90, Height = 28, BackColor = Color.FromArgb(30, 70, 40), ForeColor = Color.White, FlatStyle = FlatStyle.Flat, Cursor = Cursors.Hand };
            btnExport.Click += (s, e) => ExportText();
            _chkHighlight = new CheckBox { Text = "تمييز النتائج", Checked = true, ForeColor = Color.FromArgb(142, 142, 147), AutoSize = true };
            _lblResultCount = new Label { Text = "0 نتيجة", ForeColor = Color.FromArgb(142, 142, 147), AutoSize = true, TextAlign = ContentAlignment.MiddleLeft };
            searchRow.Controls.AddRange(new Control[] { _txtSearch, btnCopy, btnExport, _chkHighlight, _lblResultCount });
            _toolPanel.Controls.AddRange(new Control[] { filterFlow, searchRow });
            // ═══ إحصائيات ═══
            _lblStats = new Label
