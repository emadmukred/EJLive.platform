...
namespace EJLive.Server.WinForms
{
    public partial class ATMCardPanel : Panel
    {
        ...
        private void InitializeCard()
        {
            ...
            // Header (ID + Type)
            _headerPanel = new Panel { Height = 32, Dock = DockStyle.Top, Padding = new Padding(8, 6, 8, 0) };
            _lblId   = new Label { AutoSize = true, ForeColor = Color.White,              Font = new Font("Consolas", 10f, FontStyle.Bold) };
            _lblType = new Label { AutoSize = true, ForeColor = Color.FromArgb(142,142,147), Font = new Font("Segoe UI", 8f), Dock = DockStyle.Right };
            _headerPanel.Controls.AddRange(new Control[] { _lblId, _lblType });
            // حالة
            _lblStatus = new Label
            {
            ...
            // اسم الصراف
            _lblName = new Label { Height = 18, Dock = DockStyle.Top, ForeColor = Color.FromArgb(200, 200, 200), Font = new Font("Segoe UI", 9f), TextAlign = ContentAlignment.MiddleCenter };
            // شبكة
            _lblNetwork = new Label { Height = 16, Dock = DockStyle.Top, ForeColor = Color.FromArgb(99, 99, 102), Font = new Font("Segoe UI", 7.5f), TextAlign = ContentAlignment.MiddleCenter };
            // آخر Heartbeat + Latency
            _lblHB      = new Label { Height = 16, Dock = DockStyle.Top, ForeColor = Color.FromArgb(99, 99, 102), Font = new Font("Segoe UI", 7.5f), TextAlign = ContentAlignment.MiddleCenter };
            _lblLatency = new Label { Height = 16, Dock = DockStyle.Top, ForeColor = Color.FromArgb(99, 99, 102), Font = new Font("Segoe UI", 7.5f), TextAlign = ContentAlignment.MiddleCenter };
            // آخر جورنال
            _lblLastJournal = new Label { Height = 18, Dock = DockStyle.Top, ForeColor = Color.FromArgb(150, 210, 150), Font = new Font("Segoe UI", 7.5f), TextAlign = ContentAlignment.MiddleCenter };
            // آخر خطأ (أحمر)
            _lblLastError = new Label { Height = 16, Dock = DockStyle.Top, ForeColor = Color.FromArgb(255, 100, 100), Font = new Font("Consolas", 7.5f, FontStyle.Bold), TextAlign = ContentAlignment.MiddleCenter };
            // إحصائيات
            _lblSyncStats = new Label { Height = 16, Dock = DockStyle.Bottom, ForeColor = Color.FromArgb(99, 99, 102), Font = new Font("Segoe UI", 7f), TextAlign = ContentAlignment.MiddleCenter };
            Controls.AddRange(new Control[] {
                _colorBar, _headerPanel, _lblStatus, _lblName, _lblNetwork,
                _lblHB, _lblLatency, _lblLastJournal, _lblLastError, _lblSyncStats
            });
        ...
        public void UpdateATM(ATMInfo atm)
        {
            ...
            _lblStatus.Text       = atm.GetStatusLabel();
            _lblStatus.ForeColor  = cardColor;
            _lblNetwork.Text      = $"🌐 {atm.NetworkType} | {atm.BranchName ?? "—"}";
            _lblHB.Text           = $"💓 {atm.GetElapsed(atm.LastHeartbeatUtc)} | {atm.GetElapsed(atm.LastSyncUtc)}";
            _lblLatency.Text      = $"⏱️ {atm.Latency_ms} ms";
            _lblLastJournal.Text  = !string.IsNullOrEmpty(atm.LastJournalFile) ? $"📁 {atm.LastJournalFile}" : "";
            _lblLastError.Text    = !string.IsNullOrEmpty(atm.LastErrorCode)   ? $"⚠ {atm.LastErrorCode}" : "";
            _lblSyncStats.Text    = $"✅{atm.ApprovedTransactions} ❌{atm.FailedTransactions} 💳{atm.CardsCaptured}";
            // تأثير الخلفية حسب الحالة
            BackColor = state == ATMCardState.ConnectedActive
                ? Color.FromArgb(26, 34, 26)    // أخضر داكن
                : state == ATMCardState.CriticalOffline
                ? Color.FromArgb(34, 26, 26)    // أحمر داكن
