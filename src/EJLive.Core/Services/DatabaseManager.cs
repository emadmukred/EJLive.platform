...
{
    /// <summary>
    /// Code-owned SQLite database manager: 19 tables, code-applied migrations, WAL mode
    /// and a single <see cref="SqliteConnection"/> per call. Wave 1 (D-06) migrated the
    /// implementation from <c>System.Data.SQLite</c> to <c>Microsoft.Data.Sqlite</c> so
    /// the assembly pins one ADO.NET provider (DEP-1 holds); the public surface is
    /// unchanged so <c>JournalOffsetStore</c>, the verification probes and the tests
    /// keep resolving the same symbols.
    /// </summary>
    /// <remarks>
    /// Constraints (SS-11 / DB-1, DB-2): idempotent upserts keyed by
    /// <c>(atm_id, file_name, checksum)</c> for sync_records and
    /// <c>(atm_id, stat_date)</c> for daily_stats; one PRAGMA per statement (a merged
    /// PRAGMA list is a SQLite syntax error); WAL + <c>synchronous=NORMAL</c> +
    /// <c>cache_size=4000</c> + <c>temp_store=MEMORY</c>.
    /// </remarks>
    public sealed class DatabaseManager
    {
        ...
        {
            // One PRAGMA per statement (merged PRAGMA list is a SQLite syntax error).
            var pragmas = new[]
            {
...
($id,$atm,$fn,$os,$cs,$es,$enc,$comp,$ck,$md5,$sha,$tc,$ap,$mp,$ra,$va)",
                ...
                P("$ck",   entry.Checksum),
                // safe: vendor journal/archive fingerprint kept for byte-compatibility; not a security boundary
                P("$md5",  entry.MD5Hash),
...
ORDER BY stat_date DESC",
        ...
        // ==========================================
        // Internal helpers (Microsoft.Data.Sqlite)
        // ==========================================
        ...
        private void ExecuteSchema(string sql)
        {
            ...
            lock (_queryLock)
            {
                ...
                {
                    // Ignore "already exists" and "duplicate column" — schema is idempotent.
