...
| E-19 | eight `EJLive.Core` files are machine-merge dumps: every type appears in two to four copies, object-initialiser fragments sit inside class bodies, and enums carry the `partial` modifier, which C# does not define. They are brace-balanced, so `SYN-1` passed them, and the members differ between copies, so no lossless text transform exists — CI reported them as `CS1519`/`CS1001`/`CS1002`/`CS1003` in `EJLive.Core` | blocker (build) | **C-16** `src/EJLive.Core/Enums/ATMTypes.cs` repaired (one declaration per enum, union of members, values preserved — verified member-for-member against the previous revision) with the new `tools/gates/check_merge_dumps.py`; the other eight recorded as `DEBT-LEDGER` D-08 and pinned by gate rule `SYN-5`, so the set is fixed-size and cannot regrow; Wave 1 rewrites them against real call sites |
